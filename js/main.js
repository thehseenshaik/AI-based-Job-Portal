...
(function(){
  if(reduce) return;
  var v=document.getElementById('heroVisual'), dash=v&&v.querySelector('.dash');
  if(!v||!dash) return;
  v.addEventListener('mousemove',function(e){
    var r=v.getBoundingClientRect();
    var px=(e.clientX-r.left)/r.width-.5, py=(e.clientY-r.top)/r.height-.5;
    dash.style.transform='rotateY('+(-10+px*10)+'deg) rotateX('+(5-py*8)+'deg)';
  });
  v.addEventListener('mouseleave',function(){dash.style.transform='';});
})();
